...
namespace TunnelEffect {
    ...
    public partial class TunnelFX2 : MonoBehaviour {
        ...
        //// Define an array of textures for the second layer
        //public Texture2D[] secondLayerTextures;

        //void Start()
        //{
        //    // Load all possible textures for the second layer
        //    secondLayerTextures = new Texture2D[] {
        //        GetTexture("tube7"),
        //        GetTexture("tube9"),
        //        GetTexture("tube5"),
        //        GetTexture("tube4"),
        ...
        //    // Randomly select a texture for the second layer
        //    Texture2D selectedTexture = secondLayerTextures[UnityEngine.Random.Range(0, secondLayerTextures.Length)];

        //    // Assign the selected texture to the second layer in the SpaceTravel preset
        //    _layerTextures[3] = selectedTexture;

        //    // Call the UpdateMaterialPropertiesNow method to apply the changes
        //    UpdateMaterialPropertiesNow();
        //}
        ...
        void UpdateMaterialPropertiesNow() {
            ...
            switch (_preset) {
                case TUNNEL_PRESET.SpaceTravel:
                    ...
                    //}
                    _layerTextures[0] = GetTexture("tube9");
                    _layerTextures[1] = GetTexture("starfield");
                    _layerTextures[2] = GetTexture("starfield");
                    _layerTextures[3] = GetTexture("starfield");
                    // Define different alphas for each texture
                    float[] textureAlphas = new float[] { 0.5f, 0.3f, 0.3f, 0.1f };
                    float[] travelSpeeds = new float[] { 2f, 1.2f, 0.7f, 0.25f };
                    float[] rotation = new float[] { 0.7f, 0.3f, 0.2f, 0.1f };
                    float[] twist = new float[] { 0.1f, 0.1f, 0.1f, 0.1f };
                    float[] expo = new float[] { 0.9f, 0.9f, 0.9f, 0.9f };

                    //_alpha = new float[] { 0.533328f, 0.26664f, 0.13332f, 0.06666f };
                    _alpha = textureAlphas;
                    //_travelSpeed = new float[] { 2f, 1.2f, 0.7f, 0.25f };
                    _travelSpeed = travelSpeeds;
                ...
                case TUNNEL_PRESET.MagmaTunnel:
                    ...
                    //}
                    _layerTextures[0] = GetTexture("tube6");
                    _layerTextures[1] = GetTexture("fire");
                    _layerTextures[2] = GetTexture("fire");
                    _layerTextures[3] = GetTexture("fire");

                    float[] textureAlphasMagma = new float[] { 0.4f, 0.2f, 0.2f, 0.3f };
                    float[] travelSpeedsMagma = new float[] { 4f, 3f, 8f, 5f };
                    float[] rotationMagma = new float[] { 3f, 2f, 5f, -1f };
                    float[] twistMagma = new float[] { 0.3f, 1f, 1f, -1f };
                    float[] expoMagma = new float[] { 0.6f, 0.55f, 0.6f, 0.65f };


                    //_alpha = new float[] { 0.533328f, 0.26664f, 0.13332f, 0.06666f };
                    _alpha = textureAlphasMagma;
                    //_travelSpeed = new float[] { 0.7f, 0.4f, 0.2f, 0.1f };
                    _travelSpeed = travelSpeedsMagma;
                    //_rotationSpeed = new float[] { 0f, -0.002f, 0.003f, -0.005f };
                    _rotationSpeed = rotationMagma;
                ...
                case TUNNEL_PRESET.MetalStructure:
                    _layerCount = 2;
                    _layerTextures[0] = GetTexture("metallicstructure");
                    _layerTextures[1] = GetTexture("tube3");
                    float[] textureAlphasMetallic = new float[] { 0.25f, 0.27f, 0.29f, 0.31f };
                    float[] travelSpeedsMetallic = new float[] { 20f, 5f, 5f, 5f };
                    float[] rotationMetallic = new float[] { 10f, 5f, 5f, -1f };
                    float[] twistMetallic = new float[] { 1f, 1f, 1f, -1f };
                    float[] expoMetallic = new float[] { 0.8f, 0.75f, 0.6f, 0.65f };



                    //_alpha = new float[] { 0.533328f, 0.26664f, 0.13332f, 0.06666f };
                    _alpha = textureAlphasMetallic;
                    //_travelSpeed = new float[] { 0.7f, 0.4f, 0.2f, 0.1f };
                    _travelSpeed = travelSpeedsMetallic;
                    //_rotationSpeed = new float[] { 0f, -0.002f, 0.003f, -0.005f };
                    _rotationSpeed = rotationMetallic;
                    //_twist = new float[] { 0.2f, -0.2f, 0.2f, -0.2f };
                    _twist = twistMetallic;
                    //_exposure = new float[] { 0.55f, 0.55f, 0.6f, 0.65f };
                    _exposure = expoMetallic;
                    _fallOff = 0.7f;
                ...
                case TUNNEL_PRESET.WaterTunnel:
                    ...
                    _layerTextures[0] = GetTexture("tube2");
                    _layerTextures[1] = GetTexture("water2");
                    _layerTextures[2] = GetTexture("water2");
                    _layerTextures[3] = GetTexture("lights1");
                    float[] textureAlphasWater = new float[] { 0.9f, 0.27f, 0.13f, 0.1f };
                    float[] travelSpeedsWater = new float[] { 3f, 2f, 0.5f, 5f };
                    float[] rotationWater = new float[] { 1.5f, 0.18f, 0.1f, 0f };
                    float[] twistWater = new float[] { 0f, 0.04f, 0.03f, 0.02f };
                    float[] expoWater = new float[] { 0.5f, 0.6f, 0.6f, 0.6f };


                    //_alpha = new float[] { 0.533328f, 0.26664f, 0.13332f, 0.06666f };
                    _alpha = textureAlphasWater;
                    //_travelSpeed = new float[] { 0.7f, 0.4f, 0.2f, 0.1f };
                    _travelSpeed = travelSpeedsWater;
                    //_rotationSpeed = new float[] { 0f, -0.002f, 0.003f, -0.005f };
                    _rotationSpeed = rotationWater;
                    //_twist = new float[] { 0.2f, -0.2f, 0.2f, -0.2f };
                    _twist = twistWater;
                    //_exposure = new float[] { 0.55f, 0.55f, 0.6f, 0.65f };
                    _exposure = expoWater;
                ...
                case TUNNEL_PRESET.Twightlight:
                    _layerCount = 3;
                    _layerTextures[0] = GetTexture("tube3");
                    ...
                    float[] textureAlphasTwightlight = new float[] { 0.7f, 0.27f, 0.1332f, 0.1f };
                    float[] travelSpeedsTwightlight = new float[] { 5f, 10f, 5f, 5f };
                    float[] rotationTwightlight = new float[] { 1f, 0.18f, 0.16f, 0f };
                    float[] twistTwightlight = new float[] { 1f, 1.3f, -1f, 0.02f };
                    float[] expoTwightlight = new float[] { 0.7f, 0.83f, 0.7f, 0.6f };

                    //_alpha = new float[] { 0.533328f, 0.26664f, 0.13332f, 0.06666f };
                    _alpha = textureAlphasTwightlight;
                    //_travelSpeed = new float[] { 0.7f, 0.4f, 0.2f, 0.1f };
                    _travelSpeed = travelSpeedsTwightlight;
                    //_rotationSpeed = new float[] { 0f, -0.002f, 0.003f, -0.005f };
                    _rotationSpeed = rotationTwightlight;
                    //_twist = new float[] { 0.2f, -0.2f, 0.2f, -0.2f };
                    _twist = twistTwightlight;
                    //_exposure = new float[] { 0.55f, 0.55f, 0.6f, 0.65f };
                    _exposure = expoTwightlight;
                    _fallOff = 0.5f;
                ...
                case TUNNEL_PRESET.Chromatic:
                    ...
                    float[] rotationChromatic = new float[] { -1f, 0.18f, 0.1f, 0f };
                    float[] twistChromatic = new float[] { -1f, 0.04f, 0.03f, 0.02f };
                    
                    _layerTextures[0] = GetTexture("water");
 

                    _alpha = new float[] { 0.533328f, 0.26664f, 0.13332f, 0.06666f };
                    //_alpha = textureAlphasChromatic;
                    _travelSpeed = new float[] { 0.7f, 0.4f, 0.2f, 0.1f };
                    //_rotationSpeed = new float[] { 0f, -0.002f, 0.003f, -0.005f };
                    _rotationSpeed = rotationChromatic;
                    //_twist = new float[] { 0.2f, -0.2f, 0.2f, -0.2f };
                    _twist = twistChromatic;
                    
                    _exposure = new float[] { 0.55f, 0.55f, 0.6f, 0.65f };
                    //_exposure = expoChromatic;
                    _fallOff = 1f;
                    _backgroundColor = Color.white;
                    _positionAnimated = false;
                    _animationAmplitude = 0.05f;
