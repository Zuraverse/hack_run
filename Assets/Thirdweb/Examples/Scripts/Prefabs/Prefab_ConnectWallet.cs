...
public class Prefab_ThirdwebConnect : MonoBehaviour
{
    ...
    public async Task CheckWalletConnection(WalletConnection wc = null)
    {
        ...
        if (isConnected)
        {
            // Perform the actions that you want to replicate from the first connection
            _address = await sdk.wallet.GetAddress();
            Debug.Log("SSSS " + _address);
            ThirdwebDebug.Log($"Connected to {_address}");
            PlayerPrefs.SetString("WalletAddress", _address);
            PlayerPrefs.Save();
        ...
        {
            Debug.Log("Wallet is Not Connected");
            //_address = null;
            //_password = null;
    ...
    public void ToggleWalletType()
    {
        // Toggle the useSmartWallets boolean
        useSmartWallets = !useSmartWallets;

        // Disconnect the current wallet
        Disconnect();

        // Connect to the new wallet type
        if (useSmartWallets)
        {
            // Connect to the smart wallet
            ConnectExternal(WalletProvider.SmartWallet.ToString());
        }
        else
        {
            // Connect to the normal wallet
            ConnectExternal(WalletProvider.LocalWallet.ToString());
        }
    }
}
