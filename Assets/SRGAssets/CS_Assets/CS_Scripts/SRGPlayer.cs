	...
	public class SRGPlayer:MonoBehaviour 
	{
        ...
        void Update()
        {
            ...
            if (shieldDuration > 0)
            {
                ...
                // Keep emitting the shield effect particles
                if (shieldEffect != null && shieldEffect.emissionRate != 2)
                    shieldEffect.emissionRate = 2;

                // Emit particles from the player if the playerShieldEffect is not null
                if (shieldEffect != null && !shieldEffect.isPlaying)
                    shieldEffect.Play();
            }
            else
            {
                // Stop emitting shield effect particles
                if (shieldEffect != null && shieldEffect.emissionRate != 0)
                    shieldEffect.emissionRate = 0;

                // Stop emitting particles from the player
                if (shieldEffect != null && shieldEffect.isPlaying)
                    shieldEffect.Stop();
            }
            ...
            {
                Time.timeScale = 1;
                var MusicSource = GameObject.Find("MusicManager");
                var SpaceshiController = GameObject.Find("GameControllerStage01");
                if (MusicSource != null && SpaceshiController != null)
                {
                    MusicSource.GetComponent<AudioSource>().pitch = 1f;
                    SpaceshiController.GetComponent<SRGGameController>().rotateSpeed = 300;
                }
            }
        }
        ...
        {
            Debug.Log("Called Actiate Shield On Player");
            Debug.Log(duration);
        ...
        public void Slowmotion(float duration)
        {
            ...
            if(MusicSource != null && SpaceshiController != null)
            {
                MusicSource.GetComponent<AudioSource>().pitch = 0.5f;
                SpaceshiController.GetComponent<SRGGameController>().rotateSpeed = 650;
            }
        }
