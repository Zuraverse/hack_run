...
namespace SpeederRunGame
{
	...
	{
		[Header("Leaderboard Script")]
		//[Tooltip("Attach the leaderboard script")]
		//public PlayFabLeaderboardManager playFabLeaderboardManager;
		//public LeaderboardHouses playFabLeaderboardManager;
...
//CALCULATING section CHANCES
		...
		public void Pause( bool showMenu )
		{
			...
			if ( showMenu == true )
			{
				if (pauseCanvas)
				{
                    ...
                    AddSurface.SetActive(true);
					AddSurface.GetComponent<VideoAdSurface>().ServeAd();
                }
		...
		public void  Unpause()
		{
			...
			if (pauseCanvas)
			{
				pauseCanvas.gameObject.SetActive(false);
                AddSurface.SetActive(false);
                AddSurface.GetComponent<VideoAdSurface>().DisableAd();
            }
			if ( gameCanvas )    gameCanvas.gameObject.SetActive(true);
		}
		...
		IEnumerator GameOver(float delay)
		{
			...
			if ( gameOverCanvas )    
			{
				...
				// Get the count of ids in NFTData
				//int idsCount = NFTData.Instance.ids.Count;
				// Check the value of ids and call SendLeaderboardd accordingly
				foreach (int id in NFTData.Instance.ids)
				{
					switch (id)
					{
						case 0:
							LeaderboardHouses.Instance.SendDaturaLeaderboardd((int)score);
							Debug.Log("Datura Leaderboard");
							break;
						case 1:
							LeaderboardHouses.Instance.SendIbogaLeaderboardd((int)score);
							Debug.Log("Iboga Leaderboard");
							break;
						case 2:
							LeaderboardHouses.Instance.SendPeyoteLeaderboardd((int)score);
							Debug.Log("Peyote Leaderboard");
							break;
						case 3:
							LeaderboardHouses.Instance.SendAyahuascaLeaderboardd((int)score);
							Debug.Log("Ayahuasca Leaderboard");
							break;
						case 4:
							LeaderboardHouses.Instance.SendKavaLeaderboardd((int)score);
							Debug.Log("Kava Leaderboard");
							break;
						default:
							Debug.LogWarning("Invalid id value: " + id);
							break;
					}
				}
				LeaderboardHouses.Instance.SendLeaderboardd((int)score);
				Debug.Log("World Leaderboard");
